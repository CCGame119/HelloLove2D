...
---@field private _resBundle Love2DEngine.AssetBundle
...
function UIPackage.CreateObjectFromURL(url, userClass)    
    local pi = UIPackage.GetItemURL(url)
    if nil ~= pi then
...
function UIPackage:UnloadAssets()
    ...
    if nil ~= self._resBundle then
        self._resBundle:Unload(true)
        self._resBundle = nil
    end
end

---@overload fun()
---@param resBundle Love2DEngine.AssetBundle
function UIPackage:ReloadAssets(resBundle)
    if self._fromBundle and nil == resBundle then
        error("FairyGUI: new bundle must be passed to this function")
    end

    self._resBundle = resBundle
    self._fromBundle = self._resBundle ~= nil

    for _, pi in ipairs(self._items) do
        if (pi.type == PackageItemType.Atlas) then
            if (pi.texture ~= nil and pi.texture.nativeTexture == nil) then
                self:LoadAtlas(pi)
            end
        elseif (pi.type == PackageItemType.Sound) then
            if (pi.audioClip ~= nil and pi.audioClip.nativeClip == nil) then
                self:LoadSound(pi)
            end
        end
    end
...
function UIPackage:LoadAtlas(item)
    ...
    tex, dm = self._loadFunc(item.file, AssetType.tex2d)
    if nil == tex then
...
---@param self FairyGUI.UIPackage
__get.resBundle = function(self) return self._resBundle end
