...
---@field public displayObject FairyGUI.DisplayObject @Lowlevel display object.
...
function GObject:__ctor()
    ...
    self.id = "_n" .. GObject._gInstanceCounter
    GObject._gInstanceCounter = GObject._gInstanceCounter + 1
...
function GObject:SetPosition(xv, yv, zv)
    if (self._x ~= xv or self._y ~= yv or self._z ~= zv) then
        ...
        if (GObject.draggingObject == self and not GObject.sUpdateInDragging) then
